...
function adTypeStatus($type_status){
    if($type_status == 1){
        $type = "Interstitial";
    }
...
function DeviceTypeStatus($type_device){
    if($type_device == 1){
        $type = "Android";
    }
    elseif($type_device == 2){
        $type = "iOS";
    }
...
function sendPushNotification_customers($data)
{
        ...
        $RegIdChunks = array_chunk($tokens_android,1000);
